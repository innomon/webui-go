<script lang="ts">
	...
	export const replaceCommandWithText = async (text) => {
		...
		if (insertPromptAsRichText) {
			...
			const newPos =
				start +
				nodesToInsert.reduce((sum, node) => sum + node.nodeSize, 0);
			...
			if (text.includes("\n")) {
				...
				tr = tr.setSelection(
					TextSelection.near(tr.doc.resolve(newSelectionPos)),
				);
			} else {
				tr = tr.replaceWith(
					start,
				...
				tr = tr.setSelection(
					state.selection.constructor.near(
						tr.doc.resolve(start + text.length + 1),
					),
				);
			}
		}
	...
	onMount(async () => {
		...
		if (collaboration && documentId && socket && user) {
			const { SocketIOCollaborationProvider } =
				await import("./RichTextInput/Collaboration");
			...
			editorProps: {
				...
				handlePaste: (view, event) => {
					...
					if (!richText) {
						...
						dispatch(
							state.tr
								.replaceSelectionWith(fragment, false)
								.scrollIntoView(),
						);
				...
				handleDOMEvents: {
						...
						if (messageInput) {
							...
							function isInside(nodeTypes: string[]): boolean {
								...
								while (currentNode) {
									if (
										nodeTypes.includes(
											currentNode.parent.type.name,
										)
									) {
										return true;
									}
									if (!currentNode.depth) break; // Stop if we reach the top
									currentNode = state.doc.resolve(
										currentNode.before(),
							...
							if (event.key === "Tab") {
								...
								if (isInCodeBlock) {
									...
									const handled = selectNextTemplate(
										view.state,
										view.dispatch,
									);
									if (handled) {
							...
							if (event.key === "Enter") {
								const isCtrlPressed =
									event.ctrlKey || event.metaKey; // metaKey is for Cmd key on Mac
								...
								if (event.shiftKey && !isCtrlPressed) {
									...
									); // Move viewport to the cursor
									event.preventDefault();
									return true;
								} else {
									const isInCodeBlock = isInside([
										"codeBlock",
									]);
									...
									if (
										...
										console.log(
											"Letting heading suggestion handle Enter key",
										);
										return true;
									}
								}
					...
					paste: (view, event) => {
						if (event.clipboardData) {
								...
								const isWebView =
									...
									(/wv/i.test(navigator.userAgent) || // Standard Android WebView flag
										(navigator.userAgent.includes(
											...
											!navigator.userAgent.includes(
												"Chrome",
											)) || // Other generic Android WebViews
										(navigator.userAgent.includes(
											"Safari",
										) &&
											!navigator.userAgent.includes(
							...
							const hasImageItem = Array.from(
								event.clipboardData.items,
							).some((item) => item.type.startsWith("image/"));

							const hasFile =
								Array.from(event.clipboardData.files).length >
								0;
					...
					copy: (view, event: ClipboardEvent) => {
						...
						const html = editor.schema
							? editor.getHTML(slice)
							: editor.getHTML(); // depending on your editor API
